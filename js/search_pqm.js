...
var prefix = ''; // prefix to the path saved in the xml    
var runs_hash = {};  // hash of runnumber => 1 (all pqm)
var runtype_hash = {}; // hash of runnumber => run_type (all runs in db)
var runs = []; // sorted array of run numbers (all diagnostics)
var runlist = {}; //runlist for search
var runlist_keys = []; //runlist for search
var figures = []; // sorted array of figure names 
var figures_hash = []; // hash of figname => figpath
var nPlots = 1;
...
function get_runlist_figurelist() {
    xhr = $.ajax({
        ...
        success: function(xml) {
            ...
            var xhr_2 = $.ajax({
                type: "POST",
                url: "pqm_xml_figurelist",
                dataType: "xml",
                success: function(xml) {
                    var figure_count = 0;
                    $(xml).find('figname').each(function(){
                        var figname = $(this).text();
                        figures[figure_count] = figname;
                        figures_hash[figname] = $(this).next('path').text();
                        figure_count++;
                    });
                },
                complete: function() {
...
$("#submit").click( function() {
    ...
    for (i=0; i<runlist_keys.length; i++) {
        ...
        $('.pblabel').text('Run ' + run + ' Loaded');
        var percent = (i+1)*100 / runlist_keys.length;
